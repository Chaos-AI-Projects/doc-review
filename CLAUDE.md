...
Line-anchored markdown review web app. Lets a reviewer read markdown source files in the browser and attach comments anchored to specific lines.

## Quick Reference

```bash
# Install dependencies (Python 3.10+)
pip install fastapi uvicorn jinja2 markdown httpx python-multipart
...
- **Storage:** SQLite (`comments.db`) for line-anchored comments
- **File identity:** git blob object id for git-tracked files; path+content SHA-256 fallback
- **Rendering:** Markdown source rendered to HTML per-line with anchors
...
├── renderer.py        # Markdown → per-line HTML renderer
